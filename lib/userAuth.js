...
const userAuth = async (req, res, next) => {
    ...
    if (type === 'SecurityQuestion') {
        const credentials = Buffer.from(payload, 'base64').toString('ASCII')
        const [username, securityQuestion1, securityQuestion2, securityQuestion3] = credentials.split(':')
        const validUserCredentials = await verifyUserSecurityQuestions(username, securityQuestion1, securityQuestion2, securityQuestion3)
        console.log('valid user', validUserCredentials)
        if (validUserCredentials) {
            req.username = username
            req.role = validUserCredentials
